# myleprocess.py
# CS 158A, Assignment 4: Leader Election
...
class Node:
    ...
    def __init__(self, log_file_name):
        ...
        self.log(f"Logging to {self.log_file_name}")
    ...
    def log(self, message):
        ...
        with self.log_lock:
            try:
                ...
                print(f"[{self.log_file_name}] {message}")
            except IOError as e:
    ...
    def process_message(self, msg):
        ...
        with self.state_lock:
            ...
            if msg.flag == 1:
                self.log(f"Announcement received. Leader is {msg.uuid}")
                self.state = 1
...
def find_next_log_file():
    """
    Finds the next available log file name (e.g., log1.txt) in a
    race-condition-safe manner.
    """
    log_num = 1
    while True:
        log_file_name = f"log{log_num}.txt"
        try:
            # Automatically create the file. This operation fails if the file
            # already exists, preventing two processes from using the same file.
            fd = os.open(log_file_name, os.O_WRONLY | os.O_CREAT | os.O_EXCL)
            os.close(fd)
            return log_file_name
        except FileExistsError:
            # This name is taken, try the next number.
            log_num += 1

def main():
    log_file_path = find_next_log_file()
